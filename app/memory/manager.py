...
class MemoryManager:
    ...
    async def search_group_memory(
        ...
        return await self.search_group_memories(
            group_telegram_id=group_telegram_id,
            search_text=query,
            limit=limit,
        )
        ...
        async with SessionFactory() as session:
            try:
                ...
                if content is not None:
                    ...
                    if not normalized:
                        raise ValueError(
                            "Group memory content bo‘sh bo‘lishi mumkin emas."
                        )
    ...
    async def restore_group_memory(
        self,
        group_telegram_id: int,
        memory_id: int,
    ) -> bool:
    ...
    async def count_group_memory(
        ...
        return await self.group_memory_count(
            group_telegram_id=group_telegram_id
        )
